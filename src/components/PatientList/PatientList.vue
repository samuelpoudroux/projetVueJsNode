      ...
      <div class="card-header center">
       <h3 class='' >Patients</h3>
        ...
        <div class=" loader" v-if="isLoading !== false" > 
            ...
            <PatientCard :id="item.id" v-for="item in pageOfItems" :key="item.id" @showPopup="updateParent" />    
        </div>
        <div  v-if="isLoading === false" class="card-footer text-center">
            <jw-pagination :items="patients" @changePage="onChangePage" :pageSize="6"></jw-pagination>
        </div>
    </div>
</template>
    ...
    handleChange(search){
      if(this.search === "") {
       this.getAllpatient()
        ...
        this.loading = true
          Fetch.get("http://localhost:3000/patients/firstname/" + search)
          .then(data => {
            this.isLoading = false,
            this.loading = false,
