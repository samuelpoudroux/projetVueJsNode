...
<template>
    <div>
        <div v-if="showPopup === true ">
            <Popup text="Votre patient à bien été modifié" :patientAltered="true" />
        </div>
        <b-form class='center' @submit="onSubmit" @reset="onReset">
            <b-form-group class='form' id="input-group-1" label="Nom:" label-for="input-1" description="">
                <b-form-input id="input-1" v-model="patientData.lastName" placeholder="Enter Nom"></b-form-input>
            </b-form-group>
    
            <b-form-group class='form' id="input-group-2" label="Prénom:" label-for="input-2">
                <b-form-input id="input-2"   v-model="patientData.firstName" placeholder="Prénom"></b-form-input>
            </b-form-group>
    
            <b-form-group class='form' id="input-group-3" label="Age:" label-for="input-3">
                <b-form-input v-model="patientData.age"   id="input-2" placeholder="Age"></b-form-input>
            </b-form-group>
        ...
        postPatient() {
            ...
            Fetch.put('http://localhost:3000/addresses/' + this.addressData.id, addressBody)
                .then(addressData => {
                    ...
                    Fetch.put('http://localhost:3000/patients/' + this.patientData.id, patientBody)
                        .then(data => {
                            this.showPopup = true
                        })
                })
